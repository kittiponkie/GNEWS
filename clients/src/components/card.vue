  ...
  export default {
    ...
    async mounted() {
      ...
      await this.$getItem('News').then(function (value) {
        document.getElementById('test').innerHTML = value.length;
      })
      ...
      await this.$getItem('News').then(function (value) {
        ...
        for (var i = 0; i < value.length; i++) {
          document.getElementById("pic_" + (i + 1)).innerHTML =
            `
          ...
          document.getElementById("name_" + (i + 1)).innerHTML =
            `
              ...
              <span> ${i}</span>
            `;
          document.getElementById("url_" + (i + 1)).innerHTML =
            `
              http://localhost:8080/#/News/${value[i][2].article_id}
            `;
          document.getElementById("article_" + (i + 1)).innerHTML =
            `
